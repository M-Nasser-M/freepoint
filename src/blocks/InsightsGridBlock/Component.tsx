      ...
      <div className="container mx-auto px-4">
        ...
        <div className={`grid grid-cols-1 ${gridCols[columns || '3']} gap-8`}>
          {insights?.map((insight) => {
            ...
            const imageAlt = typeof insight.image === 'string' ? insight.title : insight.image?.alt || insight.title;
            const insightUrl = `/insights/${insight.slug}`; // Assuming a path structure

            return (
              <Link key={insight.id} href={insightUrl} passHref legacyBehavior>
                <a className="block h-full">
                  <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer flex flex-col">
                    ...
                    <CardContent className="flex-grow">
                      {insight.excerpt && (
                        <CardDescription className="line-clamp-3 mb-2">{insight.excerpt}</CardDescription>
                      )}
                      {insight.readTime && <p className="text-xs text-gray-500 mt-auto pt-2">{insight.readTime}</p>}
                    </CardContent>
                  </Card>
                </a>
              </Link>
